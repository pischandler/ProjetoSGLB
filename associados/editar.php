...
if (empty($dados['id'])) {
    ...
    try {
        ...
        if ($stmt_cpf->rowCount() > 0) {
            ...
            // Verificar se o RA já está cadastrado em outro associado
            $query_ra = "SELECT id FROM associados WHERE ra = :ra AND id != :id";
            $stmt_ra = $conn->prepare($query_ra);
            $stmt_ra->bindParam(':ra', $dados['ra']);
            $stmt_ra->bindParam(':id', $dados['id']);
            $stmt_ra->execute();

            if ($stmt_ra->rowCount() > 0) {
                // RA já está sendo usado por outro associado
